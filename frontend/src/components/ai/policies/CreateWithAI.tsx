  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const normalizedResult: AnalysisResult = {
        suggested_type: result.suggested_type === 'natural_language' ? 'natural_language' : 'logical',
        confidence: Number(result.confidence ?? 0.7),
        reason: result.reason || 'The AI generated a policy suggestion.',
        suggested_name: result.suggested_name || input.trim().slice(0, 60),
        summary: result.summary || 'Policy suggestion generated by Gemini.',
        dsl: result.dsl ?? null,
        refined_instruction: result.refined_instruction ?? null,
      ...
      setPolicyType(normalizedResult.suggested_type)
      setPolicyName(normalizedResult.suggested_name || input.trim().slice(0, 60))
      setTags(normalizedResult.suggested_tags)
      setStep(1)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to analyze policy. Please try again.'
      setError(message)
    } finally {
